...
graphiceditor::graphiceditor(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::graphiceditor),
      image_(new QPixmap(800, 600)), // Начальный размер изображения
      scrollArea(new QScrollArea(this)), currentPen(Qt::black, 2) {
  ui->setupUi(this);
...
void graphiceditor::resizeEvent(QResizeEvent *event) {
  if (event->size().width() > image_->width() ||
      event->size().height() > image_->height()) {
...
void graphiceditor::on_SetPen_triggered() {
  ...
  styleComboBox->addItem("Dash Dot Dot",
