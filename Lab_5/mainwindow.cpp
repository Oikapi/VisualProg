...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), editor(new QTextEdit),
      tableWidget(new QTableWidget), tableModified(false) {
  ui->setupUi(this);
  ...
  if (fileName.endsWith(".csv", Qt::CaseInsensitive)) {
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
      QMessageBox::warning(nullptr, QObject::tr("Ошибка"),
                           QObject::tr("Не удалось открыть CSV файл"));
      return;
    }
    ...
    if (rows == 0 || columns == 0) {
      QMessageBox::warning(
          ...
          QObject::tr("Файл CSV пуст или имеет неправильный формат"));
      return;
    }
    ...
    if (!tableWidget) {
      QMessageBox::warning(nullptr, QObject::tr("Ошибка"),
                           QObject::tr("Не удалось создать таблицу"));
      return;
    }
    ...
    for (int i = 0; i < rows; ++i) {
      ...
      if (cells.size() != columns) {
        QMessageBox::warning(
            nullptr, QObject::tr("Ошибка"),
            QObject::tr("Некорректный CSV файл: строки содержат разное "
                        "количество столбцов"));
    ...
    QString jsonFilePath =
        settingsDir.absoluteFilePath(fileInfo.fileName() + ".json");
      ...
      for (int i = 0; i < cellSettingsArray.size(); ++i) {
        ...
        for (int j = 0; j < rowSettings.size(); ++j) {
          ...
          if (item) {
            ...
            item->setBackground(
                QColor(cellSettings["backgroundColor"].toString()));
            QFont font;
    ...
    pageIndex =
        ui->tabWidget->addTab(newTableWidget, QFileInfo(fileName).fileName());
    ui->tabWidget->setCurrentIndex(pageIndex);
    ...
    int pageIndex =
        ui->tabWidget->addTab(newEdit, QFileInfo(fileName).fileName());
    ui->tabWidget->setCurrentIndex(pageIndex);
...
void MainWindow::on_SaveFile_triggered() {
  ...
  if (!currentWidget) {
    QMessageBox::warning(this, tr("Ошибка"),
                         tr("Нет открытой вкладки для сохранения"));
    return;
  }
  ...
  QString filePath = ui->tabWidget->tabToolTip(
      ui->tabWidget->currentIndex()); // Получаем путь к файлу из tabToolTip
  ...
  if (editor) {
    ...
    if (!filePath.isEmpty()) {
      ...
      if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        QMessageBox::warning(nullptr, "Ошибка",
                             "Не удалось сохранить текстовый файл");
        return;
      }
    ...
    if (!filePath.isEmpty()) {
        ...
        for (int j = 0; j < columns; ++j) {
          ...
          if (!item) {
            item = new QTableWidgetItem(
                ""); // Создаем пустую ячейку, если она не существует
      ...
      // Если файл новый, вызываем диалог сохранения
      filePath =
          QFileDialog::getSaveFileName(this, tr("Сохранить файл таблицы"), "",
    ...
    QMessageBox::warning(this, tr("Ошибка"),
                         tr("Текущая вкладка не поддерживает сохранение"));
  }
}
...
void MainWindow::on_SaveFileAs_triggered() {
  ...
  if (tableWidget) {
    ...
    filePath =
        QFileDialog::getSaveFileName(this, tr("Сохранить файл таблицы как"), "",
                                     tr("CSV Files (*.csv);;All Files (*)"));
    if (filePath.isEmpty())
      return;
    ...
    filePath =
        QFileDialog::getSaveFileName(this, tr("Сохранить файл как"), "",
                                     tr("Text Files (*.txt);;All Files (*)"));
    if (filePath.isEmpty())
      return;
  ...
  if (widget) {
    ...
    // Проверка для QTableWidget
      ...
      reply = QMessageBox::question(this, "Закрыть вкладку",
                                    "Этот файл содержит несохранённые "
                                    "изменения. Сохранить перед закрытием?",
...
void MainWindow::on_Search_triggered() {
  ...
  if (!editor) {
    QMessageBox::warning(
        this, tr("Ошибка"),
        tr("Текущая вкладка не поддерживает поиск. Создайте файл с текстом"));
    return;
  }
  ...
  QCheckBox *caseSensitiveCheckBox =
      new QCheckBox("Учитывать регистр", &searchDialog);
  layout->addWidget(caseSensitiveCheckBox);

  QCheckBox *wholeWordCheckBox =
      new QCheckBox("Искать только полные слова", &searchDialog);
  ...
  auto search = [&](bool forward) {
    ...
    if (searchText.isEmpty()) {
      QMessageBox::information(&searchDialog, "Поиск",
    ...
    if (!forward) {
      ...
      // Проверяем позицию курсора и сдвигаем его на один символ назад, чтобы
    ...
    if (wholeWordCheckBox->isChecked()) {
      ...
      //            QRegularExpression regex("\\b" +
      //            QRegularExpression::escape(searchText) + "\\b");
      QRegularExpression regex;
      if (cyrillicRegex.match(searchText).hasMatch()) {
        regex.setPattern("((?<![\\p{L}\\d_])" +
                         QRegularExpression::escape(searchText) +
                         "(?![\\p{L}\\d_]))");

        //                regex.setPattern("(?<!\\w)" +
        //                QRegularExpression::escape(searchText) + "(?!\\w)");
      } else {
        regex.setPattern("\\b" + QRegularExpression::escape(searchText) +
                         "\\b");
        //                Метасимвол \b (граница слова) может работать корректно
        //                с латинскими символами, но при поиске по кириллице он
        //                может некорректно обрабатывать границы слов. Это
        //                связано с тем, что \b ориентирован на латинские
        //                символы и цифры, и кириллические символы могут
        //                восприниматься неправиль
      }
...
void MainWindow::on_Replace_triggered() {
  ...
  if (!editor) {
    QMessageBox::warning(this, tr("Ошибка"),
                         tr("Текущая вкладка не поддерживает поиск"));
    return;
  }
  ...
  // Чекбоксы для учета регистра и полного слова
  QCheckBox *caseSensitiveCheckBox =
      new QCheckBox("Учитывать регистр", &replaceDialog);
  layout->addWidget(caseSensitiveCheckBox);

  QCheckBox *wholeWordCheckBox =
      new QCheckBox("Искать только полные слова", &replaceDialog);
  layout->addWidget(wholeWordCheckBox);
  ...
  auto replaceAll = [&]() {
    ...
    if (searchText.isEmpty()) {
      QMessageBox::information(&replaceDialog, "Поиск",
                               "Введите текст для поиска.");
      return;
    }

    // Настройка флагов поиска
    findFlags = caseSensitiveCheckBox->isChecked()
    ...
    if (!isReplaced) {
      QMessageBox::information(&replaceDialog, "Заменить",
...
void MainWindow::closeEvent(QCloseEvent *event) {
  ...
  // Начнём с конца списка вкладок, чтобы корректно закрывать их без сбоя
  // счётчика
  for (int i = ui->tabWidget->count() - 1; i >= 0; --i) {
    ...
    if (editor && editor->document()->isModified()) {
      ...
      QMessageBox::StandardButton reply = QMessageBox::question(
          ...
          tr("Файл \"%1\" содержит несохранённые изменения. Сохранить?")
              .arg(fileName.isEmpty() ? "Новый файл"
...
void MainWindow::on_Palette_triggered() {
  ...
  editor = qobject_cast<QTextEdit *>(ui->tabWidget->currentWidget());
  QTableWidget *table =
      qobject_cast<QTableWidget *>(ui->tabWidget->currentWidget());
  ...
  if (editor) {
    ...
    QColor newTextColor = QColorDialog::getColor(currentForegroundColor, this,
                                                 tr("Выберите цвет текста"));

    // Открываем диалог выбора цвета фона
    QColor newBackgroundColor = QColorDialog::getColor(
        currentBackgroundColor, this, tr("Выберите цвет фона"));

    // Если текстовый цвет не выбран, оставляем текущий или устанавливаем чёрный
    // по умолчанию
    if (!newTextColor.isValid()) {
      newTextColor = currentForegroundColor.isValid() ? currentForegroundColor
                                                      : QColor(Qt::black);
    }
    ...
    if (newTextColor == newBackgroundColor) {
      ...
      newTextColor = (newBackgroundColor.lightness() > 128) ? QColor(Qt::black)
                                                            : QColor(Qt::white);
    }
    ...
    // Открываем диалог выбора цвета текста
    QColor newTextColor = QColorDialog::getColor(
        currentItem->foreground().color(), this, tr("Выберите цвет текста"));

    // Открываем диалог выбора цвета фона
    QColor newBackgroundColor = QColorDialog::getColor(
        currentItem->background().color(), this, tr("Выберите цвет фона"));

    // Если текстовый цвет не выбран, оставляем текущий или устанавливаем чёрный
    // по умолчанию
    if (!newTextColor.isValid()) {
      newTextColor = currentItem->foreground().color().isValid()
                         ? currentItem->foreground().color()
                         : QColor(Qt::black);
    }
    ...
    if (editor) {
      ...
      if (!selectedItems.isEmpty()) {
        foreach (QTableWidgetItem *item, selectedItems) {
          item->setFont(
              font); // Устанавливаем шрифт для каждой выбранной ячейки
        }
...
void MainWindow::saveTextSettings(const QString &filePath) {
  ...
  QString jsonFilePath =
      settingsDir.absoluteFilePath(fileInfo.fileName() + ".html");
...
void MainWindow::on_Table_triggered() {
  ...
  QRadioButton *textEditorOption =
      new QRadioButton(tr("Вставить в текстовый редактор"), &dialog);
  QRadioButton *widgetOption =
      new QRadioButton(tr("Создать таблицу как виджет"), &dialog);
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    if (textEditorOption->isChecked()) {
      ...
      int index = ui->tabWidget->addTab(
          tableWidget, tr("Таблица %1").arg(ui->tabWidget->count() + 1));
...
void MainWindow::onTableCellChanged(int row, int column) {
  ...
  QTableWidget *currentTable =
      qobject_cast<QTableWidget *>(ui->tabWidget->currentWidget());
  if (currentTable) {
    currentTable->setProperty("modified",
                              true); // Устанавливаем свойство modified в true
  }
}
  ...
  if (QTableWidget *tableWidget = qobject_cast<QTableWidget *>(currentWidget)) {
    ...
    if (cursor.currentTable()) {
      ...
      QMessageBox::warning(this, "Ошибка",
                           "Текущая ячейка не является таблицей.");
    }
  }
}
...
void MainWindow::on_DeleteRow_triggered() {
  ...
  if (QTableWidget *tableWidget = qobject_cast<QTableWidget *>(currentWidget)) {
    ...
    if (QTextTable *table = cursor.currentTable()) {
      ...
      QMessageBox::warning(this, "Ошибка",
                           "Текущая ячейка не является частью таблицы.");
    }
  }
}
...
void MainWindow::on_Paddins_triggered() {
  ...
  if (!tableWidget) {
    QMessageBox::warning(this, "Ошибка",
                         "Текущая вкладка не является таблицей.");
    return;
  }
  ...
  if (currentRow == -1 || currentColumn == -1) {
    QMessageBox::warning(this, "Ошибка",
                         "Выберите ячейку для изменения выравнивания.");
    return;
  }
